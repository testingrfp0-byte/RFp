...
async def assign_multiple_reviewers(
    ...
    return await assign_multiple_review(request, db, current_user)

@router.get("/assigned-reviewers/{file_id}", response_model=list[ReviewerOut])
async def get_reviewers_by_file(file_id: int, db: AsyncSession = Depends(get_db)):
    return await get_reviewers_by_file_service(file_id, db)
